...
const orderReducer = (state = cloneDeep(initialState.order), action) => {
    ...
    // --------------GET ORDER LIST FROM  INTERNET ----------
        ...
        case RECEIVE_UPDATE_ORDER: {
            const newData = state.data.map((order) => (order.orderId === action.payload.orderId ? action.payload : order));
            return {
                isFetching: false,
                error: undefined,
                data: newData,
        ...
        case RECEIVE_DELETE_ORDER: {
            const newData = state.data.filter((order) => order.orderId !== action.payload);
            return {
                isFetching: false,
                error: undefined,
                data: newData,
